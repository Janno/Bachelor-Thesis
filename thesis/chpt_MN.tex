...
\begin{definition} 
    \label{Myhill}
    Let $\equiv$ be an equivalence relation. $\equiv$ is a \textbf{Myhill}%
...
\begin{definition}
    ...
    Let $\equiv$ be an equivalence relation. Let $L$ be a language. We say that $\equiv$ is a \textbf{weak Nerode relation} on $L$ if and only if
    \begin{equation*}
...
\begin{theorem}{(Myhill-Nerode)}
    \label{Myhill-Nerode}
    Let $L$ be a language. The following four statements are equivalent \cite{DBLP:books/daglib/0088160}:
    \begin{enumerate}
...
We will prove that if there is a weak Nerode relation on a language $L$, the Nerode relation is of finite index.
This proves step $(3) \implies (4)$ and thus step \tcircled{c} of our plan.
...
\paragraph{}
This concludes step $(3) \implies (4)$ of theorem \ref{Myhill-Nerode} (step \tcircled{c}). 
...
\begin{definition}
    ...
    We define $f: \Sigma^* \mapsto Q_c$ such that $f_M(U)$ is the last state in $\sigma$, i.e.
    \begin{equation*}
...
\begin{proof}
    Let $u,v \in \Sigma^*$ such that $u =_{f_M} v$.
    By definition of $f_M$, the runs $u$ and $v$ on $A$ end in the same stat.
...
\begin{proof}
    ...
    This follows from lemma \ref{f_M_right_congruent} and lemma \ref{f_M_refines}.
\end{proof}
...
\paragraph{}
This concludes the proof of step $(1) \implies (2)$ (\tcircled{a}).
...
In fact, we will prove that any Myhill relation \textbf{is} a weak Nerode relation.
...
\paragraph{}
This concludes step $(2) \implies (3)$ (\tcircled{b}).

\subsection{Nerode relations to Finite Automata}
Finally, we will prove the last step of theorem \ref{Myhill-Nerode}. 
...
\paragraph{}
In order to show that $A$ accepts the language $L$, we first need to connect runs on $A$ to the equivalence classes, i.e. the domain of $f$.
...
\paragraph{}
This concludes the last step of theorem \ref{Myhill-Nerode}, namely $(4) \implies (1)$ (\tcircled{d}), and, consequently, this chapter.
